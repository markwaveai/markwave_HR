...
import { CustomDatePicker as CustomDatePicker } from '../index';
...
const LeaveScreen = ({ user }) => {
    ...
    const calculateBalances = () => {
        const newBalances = Object.keys(LEAVE_CONFIG).map(typeCode => {
            const config = LEAVE_CONFIG[typeCode];
    ...
    const handleApply = async () => {
        if (!fromDate || !reason || notifyTo.length === 0) {
            showAlert("Validation", "Please fill all required fields (Dates, Reason, Notify To)");
            return;
        }

        if (!EMPLOYEE_ID) {
            showAlert("Error", "User session invalid. Please relogin.");
            return;
        }
        ...
        if (end < start) {
            showAlert("Error", "To Date cannot be earlier than From Date.");
            return;
        }
        ...
        try {
            ...
            const msg = error.message || "Failed to apply leave";
            showAlert("Error", msg);
        } finally {
    ...
    return (
        <View style={styles.container}>
            ...
            >
                {/* ... Keep Balance Grid and History List same  ... */}
                        ...
                        <View key={index} style={styles.balanceCard}>
                            <View style={styles.balanceHeader}>
                                <View style={[styles.iconBox, { backgroundColor: item.bg }]}>
                                    <Icon
                                        name={item.code === 'cl' ? 'send' : item.code === 'sl' ? 'thermometer' : 'map'}
                                        size={16}
                                        color={item.color}
                                    />
                                </View>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView style={styles.formContainer}>
                            ...
                            >
                                ...
                                <Icon name="chevron-down" size={12} color="#636e72" />
                            </TouchableOpacity>
                            ...
                            <View style={{ flexDirection: 'row', gap: 10 }}>
                                <View style={{ flex: 1 }}>
                                    ...
                                    >
                                        ...
                                        <Icon name="calendar" size={16} color="#48327d" />
                                    </TouchableOpacity>
                                </View>
                                <View style={{ flex: 1 }}>
